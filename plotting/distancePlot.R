...
get_merged_participant_distance_info <- function(data_or_results, participant_info) {
  ...
  merged_data <- merged_data %>%
    ...
    rename(fVpx = fVpx_check) %>%
    select(-fVpx_calibration, -widthVpx, -fvpx_over_width_tmp, -calibration_check_ratio_tmp, factorVpxCm) %>%
      ...
      #distanceCalibrationTJSON has been renamed to distanceCalibrationJSON
        tmp <- tibble(
          ...
          mutate(
            ...
            fVpx = ifelse(is.finite(ipdCm) & is.finite(ipdVpx) & is.finite(eyeToFootCm),
                          eyeToFootCm * ipdVpx / ipdCm, NA_real_),
            factorVpxCm = ifelse(is.finite(fVpx) & is.finite(ipdVpx),
                                 fVpx * ipdCm, NA)
          )
